...
def load_and_prep(filepath):
    ...
    # FIX 1: Reset index to ensure 0..N alignment with matrix indices
...
def solve_matrix(df, model, tokenizer, max_len):
    n = len(df)
    print(f"Computing Compatibility Tensor for {n} users")
    ...
    # FIX 2: Correct Broadcasting for Gender Mask
    genders = np.array(df['Gender'].tolist())
    # This creates an (N, N) matrix where True if genders are DIFFERENT
    gender_mask = (genders[:, None] != genders[None, :]).astype(float)
    
    # SAFETY: If dataset has only one gender (e.g. all Males), mask is all 0s. 
    # Switch to open matching to prevent "Everyone Single" error.
    if np.sum(gender_mask) == 0:
        print("⚠️ Warning: No opposite-gender pairs found. Switching to Open Match Mode.")
        gender_mask = np.ones((n, n))
    ...
    for i in range(n):
        for j in range(n):
            if i != j:
                flames_tiebreaker[i, j] = calculate_flames(str(names[i]), str(names[j]))
    ...
    while len(taken) < n:
        ...
        if i not in taken and j not in taken:
            ...
            # Using integer indices i and j which match df index
...
if __name__ == "__main__":
    ...
    try:
        ...
        final_data = []
        # iterrows yields (index, series). Since we reset_index, idx matches logic.
